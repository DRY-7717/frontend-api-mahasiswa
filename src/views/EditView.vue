<template>
  <div class="container">
    <h2 class="text-center mt-5 mb-5">Edit Data Students</h2>
...
const updateData = () => {
  axios
    .put("http://127.0.0.1:8000/api/mahasiswa/"+id, students)
...
onMounted(() => {
  axios
    ...
    .then(function (response) {
      students.name = response.data.data[0].name;
      students.nim = response.data.data[0].nim;
      students.date_birth = response.data.data[0].date_birth;
      students.address = response.data.data[0].address;
    })
    .catch(function (error) {
      // handle error
      console.log(error);
